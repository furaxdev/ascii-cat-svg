...
const TOOLS = [
  ...
  },
  {
    title: 'Visit counter',
    path: '/visits',
    params: '?key=my-profile&colors=green',
    query: '?key=<pick any unique name for yourself>&colors=green|blue|purple — each key gets its own independent count (capped at 1/visitor/day, cached 1h)',
  },
  {
    title: 'Local clock',
    path: '/clock',
  ...
  },
  {
    ...
    path: '/discord',
    params: '?id=1261769857188495524&colors=blue',
    query: '?id=<discord user id>&colors=green|blue|purple — via Lanyard, requires joining discord.gg/lanyard (cached 30s)',
  },
  {
    title: 'Spotify now playing',
    path: '/spotify',
    params: '?user=REPLACE_ME&colors=green',
    query: '?user=<pick any unique name>&colors=green|blue|purple — first connect once at /spotify/login?user=<same name> (cached 30s)',
    noLivePreview: true,
  },
];
...
function buildShowcaseHtml() {
  const cards = TOOLS.map((t) => {
    ...
    const preview = t.noLivePreview
      ? `<span style="color:#6a6a6a;font-size:0.85em;">connect your own account first — see the query below</span>`
      : `<img src="${imgUrl}" alt="${escapeHtml(t.title)}" />`;
    return `
      <section class="card">
