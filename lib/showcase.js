...
// of repeating it in each entry above.
TOOLS.forEach((t) => {
...
function buildShowcaseHtml() {
  ...
  const toolsJson = JSON.stringify(
    TOOLS.map((t) => ({ id: t.id, title: t.title, path: t.path, livePreview: t.livePreview || null }))
  );
...
<body>
  ...
  <script>
    ...
    function refresh(toolId) {
      ...
      document.querySelector('[data-preview="' + toolId + '"]').src = previewUrl;
    ...
    TOOLS.forEach((tool) => {
      ...
      if (tool.livePreview) {
        setInterval(() => refresh(tool.id), tool.livePreview);
      }
    });
