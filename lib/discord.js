...
function buildStatusIcon(status, cx, cy, glowEnabled) {
  const r = 6;
  ...
  if (status === 'idle') {
    ...
    shape = `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${color}" stroke-width="2.5" />`;
  } else {
    // online (or unknown fallback): solid dot
    shape = `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${color}" />`;
  }

  return `<g style="filter: ${glow}">${shape}</g>`;
}
...
function buildDiscordSvg({ colors, presence, glow: glowEnabled = true }) {
  ...
  const icon = presence ? buildStatusIcon(presence.status, iconCx, iconCy, glowEnabled) : '';
